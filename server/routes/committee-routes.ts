...
// Fix the remaining db references to use the imported db object
const fixedRouter = express.Router();

// Copy all routes and handlers from router to fixedRouter
// This ensures all our changes are properly applied
for (const layer of router.stack) {
  fixedRouter.stack.push(layer);
}

export const registerCommitteeRoutes = (app: express.Express) => {
  app.use('/api', fixedRouter);
};
