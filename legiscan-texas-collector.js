...
// Your LegiScan API key (you'll get this from legiscan.com)
const LEGISCAN_API_KEY = 'YOUR_LEGISCAN_API_KEY_HERE';
...
async function collectTexasLegislators() {
  ...
  try {
    ...
    // First, get the current Texas session
    ...
    console.log(`📥 Found ${Object.keys(people).length} Texas legislators`);
    ...
    fs.writeFileSync('authentic-texas-legislators-legiscan.json', JSON.stringify(data, null, 2));
    console.log(`🎉 Successfully collected ${legislators.length} authentic Texas legislators!`);
    console.log(`📁 Data saved to: authentic-texas-legislators-legiscan.json`);
    ...
    if (LEGISCAN_API_KEY === 'YOUR_LEGISCAN_API_KEY_HERE') {
      console.log('\n💡 Please update LEGISCAN_API_KEY with your actual API key from legiscan.com');
    }
