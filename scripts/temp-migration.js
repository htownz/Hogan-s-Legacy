...
// scripts/push-scout-bot-extended-schema.ts
var import_dotenv = __toESM(require_main(), 1);
import_dotenv.default.config();
async function createEnumTypes(migrationClient) {
  try {
...
async function createExtendedScoutBotTables(dbClient) {
  try {
...
async function main() {
  ...
  const migrationClient = src_default(process.env.DATABASE_URL, { max: 1 });
