    ...
    <div className="container py-6 max-w-7xl mx-auto">
                        ...
                        <TableRow key={report.id}>
                          <TableCell>
                            {report.contentType === 'thread' ? (
                              ...
                              <Badge variant="outline" className="flex items-center gap-1">
                                <MessageCircle className="h-3 w-3" /> Post
                              </Badge>
                            )}
                          </TableCell>
                          <TableCell className="max-w-xs">
                          ...
                          <TableCell>
                            <Badge 
                              variant={
                                ...
                                report.status === 'investigating' ? 'default' : 'destructive'
                              }
                            >
                              {report.status}
                            </Badge>
                          ...
                          <TableCell>
                            <div className="flex gap-2">
                              <Button 
                                ...
                                size="sm"
                                onClick={() => {
                                  const contentItem = report.contentType === 'thread' 
                                    ? { id: report.contentId, title: report.contentSummary } 
                                    : { id: report.contentId, content: report.contentSummary };
                                  
                                  openModerateDialog(contentItem, report.contentType);
                                }}
                              >
                                Review
                              </Button>
                        ...
                        <TableRow key={mod.id}>
                          <TableCell className="font-medium">
                            <div className="flex items-center gap-2">
                              {mod.forumId === null && (
                                <Badge>Super Mod</Badge>
                              )}
                              User #{mod.userId}
                            </div>
                          </TableCell>
                          <TableCell>{mod.forumId ? mod.forumName : 'All Forums'}</TableCell>
                          <TableCell>User #{mod.addedBy}</TableCell>
                          <TableCell>{format(new Date(mod.createdAt), 'MMM d, yyyy')}</TableCell>
                          <TableCell className="max-w-xs">
                            <div className="line-clamp-2">{mod.notes || '-'}</div>
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
      ...
      <Dialog open={addModeratorOpen} onOpenChange={setAddModeratorOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Add a user as a moderator. You can assign them to a specific forum or make them a global moderator.
            </DialogDescription>
          </DialogHeader>
          <Form {...addModeratorForm}>
            <form onSubmit={addModeratorForm.handleSubmit(onAddModerator)} className="space-y-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Forum (Optional)</FormLabel>
                    <Select
                    ...
                    >
                      <FormControl>
                        <SelectTrigger>
                          <SelectValue placeholder="Select a forum (leave empty for all forums)" />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea {...field} placeholder="Add notes about this moderator" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
      ...
      <Dialog open={moderateDialogOpen} onOpenChange={setModerateDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Moderate {itemType === 'thread' ? 'Thread' : 'Post'}
            </DialogTitle>
          </DialogHeader>
          
          {selectedItem && (
            <div className="my-2 p-4 border rounded-md bg-muted">
              {itemType === 'thread' ? (
                <>
                  <h3 className="font-medium">{selectedItem.title}</h3>
                  <p className="text-sm mt-1 line-clamp-3">{selectedItem.content}</p>
                </>
              ) : (
                <p className="text-sm line-clamp-6">{selectedItem.content}</p>
              )}
            </div>
          )}
          ...
          <Form {...moderateContentForm}>
            <form onSubmit={moderateContentForm.handleSubmit(onModerateContent)} className="space-y-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Moderation Notes (Optional)</FormLabel>
                    <FormControl>
                      <Textarea 
                        {...field} 
                        placeholder="Provide reasons for your moderation decision"
                      />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
              ...
              <div className="flex justify-end pt-2">
                <Button 
                  ...
                  variant={
                    moderateContentForm.watch('status') === 'approved' ? 'default' :
                    moderateContentForm.watch('status') === 'rejected' ? 'destructive' : 'outline'
                  }
                >
                  {(moderateThreadMutation.isPending || moderatePostMutation.isPending) && (
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                  )}
                  Submit Decision
                </Button>
